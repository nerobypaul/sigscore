...
## 22:30 — Pre-flight integration validation for playbook activation

**Task:** Add pre-flight checks when activating playbooks so users see warnings if required integrations (e.g. Slack) are not configured, instead of actions silently skipping at runtime.

**Changes:**
- backend/src/services/playbooks.ts — Import getSlackWebhookUrl; add validateIntegrations() helper that checks required integrations for a playbook's actions; modify activatePlaybook() to return { workflow, warnings } instead of just workflow
- backend/src/routes/playbooks.ts — Destructure { workflow, warnings } from activatePlaybook(); include warnings array in 201 response
- frontend/src/components/Toast.tsx — Add 'warning' toast type with amber styling and triangle-exclamation icon
- frontend/src/pages/Playbooks.tsx — Capture activation response data; show each warning as a toast.warning() after successful activation

**Decisions:**
- Warnings are non-blocking: playbooks still activate even if Slack is unconfigured (user can set it up later)
- Added a full 'warning' toast type (amber/yellow) to the Toast system rather than reusing 'info' — warnings are distinct from informational messages and deserve their own visual treatment
- validateIntegrations is structured to easily add more integration checks (e.g. email, webhooks) in the future

**Status:** completed

## 22:30 — Fix 8 critical product bugs from Paul's first usage review

**Task:** Paul tested the product for the first time and found ~15 issues. Fixed the most critical ones.

**Changes:**
- backend/src/services/workflows.ts — Add interpolateTemplate() + interpolateActionParams() for {{variable}} resolution in workflow actions; integrate into processEvent() action loop
- frontend/src/components/Layout.tsx — Change /billing from hideInDemo to disabledInDemo so demo users see it with Pro badge
- backend/src/services/enrichment-queue.ts — Replace in-memory Map with DB-backed persistence (org.settings + contact.customFields); batch results survive restarts
- backend/src/routes/enrichment-queue.ts — Update route handlers for async DB-backed service functions
- backend/src/routes/enrichment.ts — Graceful degradation when API key missing; new /profile endpoint assembles data from existing DB records
- frontend/src/pages/EnrichmentQueue.tsx — Contact profile panel, improved detail view with persisted data
- frontend/src/pages/IntegrationDetail.tsx — Replace disabled "coming soon" inputs with functional config forms; OAuth integrations show actual blocking reason
- backend/src/controllers/auth.ts — Clear "already exists" error message for duplicate email registration (409 status)
- frontend/src/pages/Activities.tsx — Activity detail modal with edit/delete (committed earlier)

**Decisions:**
- Enrichment persistence uses org.settings JSON for batch metadata + contact.customFields for per-contact results (avoids schema migration)
- Integration config forms use existing POST /sources endpoint — no backend changes needed
- Duplicate email returns 409 with clear message (acceptable for B2B SaaS, not a security concern)
- Calendar/email integration for Activities deferred — requires OAuth app setup from Paul

**Status:** completed

---

## 23:00 — Signal anomaly detection (core intelligence feature)

**Task:** Add automatic detection of signal activity anomalies using z-score analysis against 30-day rolling baseline

**Changes:**
- backend/src/services/signal-anomaly.ts — NEW: Core anomaly detection service with z-score method. Computes rolling 30-day baseline (daily signal counts), calculates mean/stddev, flags SPIKE (>2 sigma above) or DROP (>2 sigma below for active accounts). Handles edge cases: new accounts (<7 days skipped), inactive accounts (mean <1 skipped), zero-variance accounts (skipped). Includes `scanOrganizationAnomalies()` for batch org scanning and `getRecentAnomalies()` for querying stored anomalies from Notification records.
- backend/src/jobs/anomaly-detection.ts — NEW: BullMQ job processor with 24-hour cooldown per account+type to prevent alert fatigue. Uses scheduler sentinel pattern (__scheduler__) for per-org fan-out. Creates Notification records with type='signal_anomaly' and JSON metadata (anomaly type, severity, expected/actual values, z-score).
- backend/src/routes/anomalies.ts — NEW: API routes for anomaly data. GET /api/v1/anomalies (org-wide, last 7 days default), GET /api/v1/anomalies/:accountId (per-account), POST /api/v1/anomalies/scan (manual trigger).
- backend/src/jobs/queue.ts — Add ANOMALY_DETECTION queue name, AnomalyDetectionJobData interface, anomalyDetectionQueue instance
- backend/src/jobs/workers.ts — Add anomaly detection worker (concurrency: 2) with scheduler sentinel handling; also registered pre-existing alert evaluation and alert check workers that were defined but not started
- backend/src/jobs/scheduler.ts — Add hourly cron at :30 past the hour for anomaly detection
- backend/src/jobs/index.ts — Export anomalyDetectionQueue and AnomalyDetectionJobData
- backend/src/app.ts — Mount anomaly routes at /api/v1/anomalies

**Decisions:**
- Stored anomalies as Notification records (type='signal_anomaly') with JSON metadata in body field to avoid schema migration
- Severity levels: moderate (2-3 sigma), high (3+ sigma)
- DROP detection requires mean > 3 signals/day (only flags drops for genuinely active accounts)
- 24-hour notification cooldown per account + anomaly type prevents alert spam
- Hourly cron at :30 offset from score snapshots (at :00) to spread load
- Uses Prisma raw SQL for efficient daily signal grouping (DATE_TRUNC)

**Status:** completed

---

## 23:30 — Alert execution engine (AccountAlertRule evaluation)

**Task:** Implement BullMQ-based alert execution engine that evaluates AccountAlertRule conditions and fires notifications through configured channels (in-app, email, Slack)

**Changes:**
- backend/prisma/schema.prisma — Add `lastTriggeredAt DateTime?` field and `[enabled, triggerType]` composite index to AccountAlertRule model for cooldown tracking and efficient cron queries
- backend/src/jobs/alert-evaluation.ts — NEW: Core alert evaluation engine. Evaluates 6 trigger types against account state: `score_drop` (ScoreSnapshot comparison over N days), `score_rise` (same), `score_threshold` (transition detection above/below threshold), `engagement_drop` (no signals for N days), `new_hot_signal` (recent signal type matching), `account_inactive` (last signal age check). Dispatches through 3 channels: in-app (notifyOrgUsers), email (Resend to all org members), Slack (Block Kit webhook). 1-hour cooldown per rule via lastTriggeredAt to prevent duplicate alerts.
- backend/src/jobs/alert-check-cron.ts — NEW: Periodic evaluator for time-based rules (engagement_drop, account_inactive) that cannot be triggered by score changes. Uses scheduler sentinel pattern (__scheduler__) for per-org fan-out — only queries orgs with active time-based rules.
- backend/src/jobs/queue.ts — Add ALERT_EVALUATION and ALERT_CHECK queue names, AlertEvaluationJobData and AlertCheckJobData interfaces, queue instances with appropriate retry config
- backend/src/jobs/workers.ts — Add createAlertEvaluationWorker (concurrency: 5) and createAlertCheckWorker (concurrency: 2); register both in startWorkers()
- backend/src/jobs/producers.ts — Add enqueueAlertEvaluation() with 5-minute time-bucketed deduplication (collapses rapid score recomputations for same account)
- backend/src/jobs/scheduler.ts — Add alert-check cron every 5 minutes for time-based rule evaluation
- backend/src/jobs/index.ts — Export new queues, types, and producer function
- backend/src/services/account-scores.ts — Hook: enqueueAlertEvaluation() called after every score computation (fire-and-forget) passing organizationId, accountId, newScore, oldScore

**Decisions:**
- 1-hour cooldown chosen to balance alert freshness vs spam — same rule won't re-fire for same account within 60 minutes
- Score-based rules evaluated per-account on score change; time-based rules evaluated across all accounts every 5 minutes (efficient fan-out)
- Alert evaluation is fire-and-forget from score computation (enqueued via BullMQ, not inline) to avoid blocking the scoring pipeline
- 5-minute deduplication bucket on alert evaluation jobs prevents flooding the queue during bulk score recomputations
- Slack messages use Block Kit format with structured fields (trigger type, score) for rich display
- Email notifications sent to all org members (future: per-user notification preferences)
- engagement_drop requires prior signal history (won't alert on brand-new accounts with zero signals)
- new_hot_signal checks 10-minute window to avoid re-alerting on older signals

**Status:** completed

---

## 24:00 — Account Health Dashboard (premium intelligence feature)

**Task:** Build comprehensive Account Health Dashboard at /health that goes beyond the existing PQA Scores page with risk/opportunity intelligence, engagement momentum, and score distribution analytics

**Changes:**
- frontend/src/pages/AccountHealth.tsx — NEW: Full Account Health dashboard with three sections:
  - Top: Health Overview with pure SVG donut chart (tier distribution), 4 stat cards (avg score, active/dormant, rising, falling), and Engagement Momentum panel with net health indicator
  - Middle: Risk & Opportunity tabbed table (At Risk: falling+score>50, Expanding: rising trend, Churning: dropped to COLD/INACTIVE with prior activity). Each row shows company name (linked), score, tier badge, trend arrow, delta, last signal time, top score factors
  - Bottom: Score Distribution histogram (pure CSS bars, 10 buckets, tier-colored with boundary legend)
- frontend/src/App.tsx — Add lazy-loaded AccountHealth import and /health route inside protected layout
- frontend/src/components/Layout.tsx — Add "Account Health" nav item as first item in ANALYSIS section with shield-check icon (AccountHealthIcon function)

**Decisions:**
- All charts are pure SVG/CSS (zero charting library dependencies, matching project convention)
- Fetches all accounts via existing /signals/accounts/top endpoint (limit: 500) and computes all stats client-side — no new backend endpoint needed
- Delta is estimated from trend direction since inline previous scores aren't available (RISING=+10, FALLING=-10, STABLE=0)
- Churning detection uses two heuristics: falling trend AND cold/inactive tier, OR high signal count (>5) with >14 days since last signal
- Shield-check icon chosen for Account Health to convey protection/monitoring differentiation from the bar-chart PQA Scores icon

**Status:** completed

---

## 24:15 — Anomaly Dashboard Frontend

**Task:** Build signal anomaly detection dashboard at /anomalies with stats, filtering, and scan trigger

**Changes:**
- frontend/src/pages/Anomalies.tsx — NEW: Full anomaly dashboard with stats bar (total/spikes/drops/avg z-score), filter tabs (All/Spikes/Drops), anomaly cards with type/severity badges, Scan Now button, linked account names, relative timestamps
- frontend/src/App.tsx — Add lazy-loaded Anomalies import and /anomalies route
- frontend/src/components/Layout.tsx — Add "Anomalies" nav item under INTELLIGENCE section

**Status:** completed

---

## 24:30 — Conditional Webhooks with Payload Templating

**Task:** Add filter conditions and custom payload templates to webhook subscriptions

**Changes:**
- backend/prisma/schema.prisma — WebhookSubscription: Added `filters Json?`, `payloadTemplate Json?`
- backend/src/services/webhook-subscriptions.ts — evaluateFilters() with AND logic (scoreAbove, scoreBelow, tiers, signalTypes, accountIds), applyPayloadTemplate() with {{variable}} interpolation, backward compatible
- frontend/src/pages/WebhookManager.tsx — Collapsible filters section (score thresholds, tier/signal type multi-select), payload template JSON editor, filter summary in subscription table

**Decisions:**
- Filters use AND logic (all conditions must match) — simpler and more predictable than OR
- No filters = unconditional dispatch (backward compatible)
- Payload template uses same {{variable}} interpolation as workflow templates

**Status:** completed

---

## 24:45 — Exponential Signal Decay Engine

**Task:** Replace binary 30-day signal window with continuous exponential decay to eliminate cliff effect

**Changes:**
- backend/src/services/account-scores.ts — Added SIGNAL_HALF_LIVES (19 types, 5-60 day half-lives), computeDecayWeight() function, 90-day scoring window (was 30), decay-weighted signal aggregation, new signal_freshness factor (7th factor, max 10pts), rebalanced PQA model to 100pts total
- backend/src/routes/scoring.ts — New GET /api/v1/scoring/decay-config endpoint with half-life map, formula description, and precomputed example curves

**Decisions:**
- Exponential decay (e^(-ln(2)/halfLife * daysOld)) chosen over linear decay for natural signal relevance modeling
- Half-lives: signup (60d), repo_fork/pull_request (28d), api_call (14d), page_view (5d) — intent correlates with durability
- Window extended to 90 days since decay handles relevance naturally (very old signals just have near-zero weight)
- PQA model rebalanced: user_count 25→20, feature_breadth 20→15, new signal_freshness 10pts

**Status:** completed

---

## 24:50 — Signal Pattern Clustering + ICP Matching

**Task:** Build signal profile analysis, lookalike detection, ICP definitions, and sequence mining

**Changes:**
- backend/src/services/signal-patterns.ts — NEW: buildSignalProfile() (type distribution, velocity, engagement pattern), findLookalikes() (cosine similarity on signal vectors + firmographic bonus), matchICP() (per-criteria scoring), detectSignalSequences() (2/3-gram mining from high-scoring accounts), ICP CRUD in org.settings JSON
- backend/src/routes/signal-patterns.ts — NEW: 7 REST endpoints at /api/v1/patterns/*
- backend/src/app.ts — Route registration
- frontend/src/pages/SignalPatterns.tsx — NEW: ICP Builder form, match results table, signal sequence visualization
- frontend/src/App.tsx — Lazy route at /patterns
- frontend/src/components/Layout.tsx — Nav item under INTELLIGENCE section

**Decisions:**
- Cosine similarity for lookalikes — lightweight, no ML library needed, scales to thousands of accounts
- ICP definitions stored in org.settings JSON to avoid new Prisma model/migration
- Sequences mined from accounts with PQA >= 70 (proven high-value)

**Status:** completed

---

## 25:00 — WebSocket Score/Tier/Anomaly Broadcasts + Subscription Filtering

**Task:** Add real-time score change, tier change, and anomaly broadcasts; allow clients to filter events

**Changes:**
- backend/src/services/websocket.ts — broadcastScoreChange(), broadcastTierChange(), broadcastAnomaly(), SubscriptionFilter interface, message handler for subscribe messages, matchesFilter() with category-based event matching
- backend/src/services/account-scores.ts — Hook broadcastScoreChange/broadcastTierChange after score computation (fire-and-forget)
- backend/src/jobs/anomaly-detection.ts — Hook broadcastAnomaly after anomaly detection

**Decisions:**
- Category-based filtering (signal, score, tier, anomaly) — clients subscribe by category, not individual event types
- Account-level filtering optional — subscribe to specific accountIds or get all
- Backward compatible — no subscribe message = receive everything

**Status:** completed

---

## 25:05 — Auto-Merge Identity Resolution at Ingestion

**Task:** Wire automatic high-confidence contact merging during signal ingestion

**Changes:**
- backend/src/services/identity-resolution.ts — autoMergeIfHighConfidence() with 24h cooldown, safety guards (sole-company-contact protection, max 2-contact merge), recordAutoMerge() (org.settings JSON, 100 FIFO cap), getAutoMergeStats()
- backend/src/services/signals.ts — Fire-and-forget autoMergeIfHighConfidence call in both ingestSignal and ingestSignalBatch
- backend/src/routes/identity.ts — GET /api/v1/identity/auto-merge-stats endpoint

**Decisions:**
- Fire-and-forget: never blocks signal ingestion (wrapped in .catch)
- 24h in-memory cooldown per contact pair avoids repeated merge attempts
- Safety: never auto-merge 3+ contacts (flags for manual review), never merge sole company contacts
- Uses existing mergeContacts() for actual merge — battle-tested, transactional

**Status:** completed

---

## 25:10 — Export History Production Bug Fix

**Task:** Move export status from in-memory Map to Redis for multi-process compatibility

**Changes:**
- backend/src/services/data-export.ts — Replaced in-memory Map with Redis (SET/GET with 24h TTL, org tracking via Redis sets)
- backend/src/routes/data-export.ts — Added await to all now-async export status calls
- backend/src/jobs/workers.ts — Added await to setExportStatus calls

**Decisions:**
- Redis chosen over database for speed and natural TTL (exports expire after 24h)
- Graceful degradation — Redis failures logged but don't crash exports
