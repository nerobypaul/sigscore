...
- backend/src/__tests__/integration/auth.integration.test.ts -- Same 400→409 fix for integration test
...
## 23:45 -- Add Resend webhook handler for email open/click tracking

**Task:** Wire up Resend webhook events so email sequences show real open rates and click rates instead of 0%.

**Changes:**
- backend/src/routes/resend-webhooks.ts -- New webhook handler at POST /api/v1/webhooks/resend. Handles email.sent, email.delivered, email.opened, email.clicked, email.bounced, email.complained events. Verifies webhook signature via HMAC-SHA256 (svix headers). Looks up EmailSend records by Resend messageId and updates status/timestamps. On bounce, marks enrollment as bounced. On complaint, unsubscribes enrollment.
- backend/src/app.ts -- Register resend webhook route alongside other unauthenticated webhook routes (no auth required, verified by signature)
- backend/src/config/index.ts -- Added email.resendWebhookSecret (RESEND_WEBHOOK_SECRET env var)
- backend/prisma/schema.prisma -- Added deliveredAt, complainedAt fields and messageId index to EmailSend model for efficient webhook correlation
- backend/src/services/email-sequences.ts -- Updated getSequenceStats to use openedAt/clickedAt timestamps instead of status for more accurate open/click counting. Added totalDelivered and totalComplained to stats response.
- frontend/src/pages/EmailSequences.tsx -- Fixed data mapping (stats are nested under .stats in API response, not flat). Fixed open/click rate display (backend returns integer percentages, not decimals).
- frontend/src/pages/EmailSequenceDetail.tsx -- Fixed stats display to use integer percentages from backend. Added totalDelivered card. Fixed completedEnrollments field name.

**Decisions:**
- Used status priority system to prevent event regression (e.g., a "delivered" event won't overwrite an "opened" status)
- On email.clicked, also set openedAt if missing (click implies open)
- Return 200 on processing errors to prevent Resend retry storms
- In production, RESEND_WEBHOOK_SECRET must be configured (returns 500 if missing)
- In development, signature verification is skipped if no secret configured

**Open questions:**
- Need to configure RESEND_WEBHOOK_SECRET in Railway env vars and register webhook URL in Resend dashboard (https://resend.com/webhooks)
- Webhook URL to register: https://<domain>/api/v1/webhooks/resend
