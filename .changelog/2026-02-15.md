...
## 32:00 -- LinkedIn connector: manual import + webhook signal ingestion

**Task:** Build a LinkedIn connector for DevSignal that tracks company page activity, post engagement, employee signals, and company follows via manual import and webhook ingestion

**Changes:**
- backend/prisma/schema.prisma -- Added LINKEDIN to SignalSourceType enum (LINKEDIN already existed in IdentityType)
- backend/prisma/migrations/20260215040000_add_linkedin_signal_source_type/migration.sql -- SQL migration for new SignalSourceType enum value
- backend/src/services/linkedin-connector.ts -- Core connector service: configureLinkedIn (stores company page URL, employee tracking toggle, auto-generated webhook secret), getLinkedInConfig, getLinkedInStatus (includes per-type signal stats via groupBy), disconnectLinkedIn, handleLinkedInWebhook (processes inbound webhook events with HMAC-SHA256 signature verification, identity resolution, signal creation, workflow trigger), importLinkedInEmployees (manual bulk import: creates/resolves contacts, creates LinkedIn ContactIdentity links, creates employee_activity signals), syncLinkedIn (housekeeping no-op for webhook-driven connector), verifyWebhookSignature (crypto.timingSafeEqual), getLinkedInConnectedOrganizations. Identity resolution: email match, LinkedIn profile URL identity, linkedIn field on Contact, name matching. Signal types: linkedin_page_view, linkedin_post_engagement, linkedin_employee_activity, linkedin_company_follow.
- backend/src/routes/linkedin-connector.ts -- REST API routes: POST /webhook/:sourceId (public, no JWT, HMAC verified via X-LinkedIn-Signature header), POST /connect (ADMIN, validates LinkedIn company/showcase URL), GET /status, POST /import (ADMIN, bulk employee import up to 500 per request with Zod validation), POST /sync (ADMIN, via BullMQ), DELETE /disconnect (ADMIN). All authenticated routes use authenticate + requireOrganization middleware.
- backend/src/jobs/queue.ts -- Added LINKEDIN_SYNC queue name, LinkedInSyncJobData interface, linkedinSyncQueue instance
- backend/src/jobs/producers.ts -- Added enqueueLinkedInSync with per-org deduplication
- backend/src/jobs/workers.ts -- Added LinkedIn sync worker with scheduler sentinel handling
- backend/src/jobs/scheduler.ts -- Added 6-hour cron for LinkedIn sync (webhook-driven, periodic housekeeping), enqueueLinkedInSyncForAllConnected helper
- backend/src/app.ts -- Mounted linkedin connector routes at /api/v1/connectors/linkedin
- frontend/src/pages/Settings.tsx -- Added LinkedIn tab (TabId + TABS array + LinkedInTab component + LinkedInIcon SVG) with: company page URL input with validation, employee tracking toggle, connected status with 5-metric signal stats grid (total, page views, post engagements, employee activity, company follows), copyable webhook URL and secret with copy buttons, webhook payload format documentation (expandable details), manual import section with dual input modes (CSV paste area + one-by-one form with queue display), last sync results, disconnect button, info box explaining how signals are captured

**Decisions:**
- LinkedIn connector is manual import + webhook driven (not API polling) since LinkedIn API requires OAuth 2.0 with a registered app and restrictive access
- Auto-generated webhook secret (whsec_ prefix + 24 random hex bytes) for HMAC-SHA256 signature verification on inbound webhooks
- Designed for integration with Zapier/Make: users connect LinkedIn triggers in Zapier and point webhooks to DevSignal's endpoint
- 6-hour sync interval since the connector is primarily webhook-driven; sync is just housekeeping
- Employee import limited to 500 per request to prevent timeout; creates contacts + LinkedIn ContactIdentity links
- Signal stats computed via Prisma groupBy for real-time accuracy (not cached)
- Company name extracted from LinkedIn URL slug for display (e.g. /company/acme-corp -> "Acme Corp")

**Open questions:**
- Once LinkedIn API access is obtained, should we add OAuth 2.0 flow for automatic page follower/engagement syncing?
- Should we add a LinkedIn Sales Navigator integration for deeper prospecting signals?

**Status:** completed

## Cumulative Integration Map
GitHub, npm, PyPI, Segment, Slack, HubSpot, Salesforce, Discord, Stack Overflow, Twitter/X, Clearbit, Reddit, PostHog, LinkedIn -- **14 data sources**
...
- Intercom/Zendesk support ticket signals
