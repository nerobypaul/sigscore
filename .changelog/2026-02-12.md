...
### P1 Sprint — Beta Readiness

**Agent 1: Prisma migrations + DataLoader**
- Generate Prisma migration for the 8 new composite indexes
- Add DataLoader for GraphQL N+1 query batching on field resolvers

**Agent 2: Node.js SDK (@devsignal/node)**
- Create packages/sdk directory with TypeScript SDK
- Signal ingest, contacts/companies/deals CRUD, PQA score queries
- Auth via API key header
- Publish-ready package.json

**Agent 3: Frontend pages — Signal Feed + PQA Dashboard**
- Signal feed page: real-time signal stream with filters
- PQA dashboard: account scores by tier, trending accounts, score history
- Connect to existing /api/v1/signals and /api/v1/signals/top-accounts endpoints

**Agent 4: Onboarding flow**
- First-run setup wizard: create organization, invite team, connect first signal source
- Empty state CTAs on all list pages pointing to onboarding
- Getting Started checklist component

### After P1: Growth Features
